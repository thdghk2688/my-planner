...
const mkTask = (r,type,pri,stat) => ({id:gid(),title:r[0],type,startDate:pND(r[1]),endDate:pND(r[1]),dueTime:"",priority:pri,status:stat,note:r[r.length-1]||""});
const INIT_TASKS = [
  ...ND.map(r => mkTask(r,"work",r[3],pStat(r[2]))),
  ...TD.map(r => mkTask(r,tType(r[2]),1,pStat(r[3]))),
];

const SKEY = "myplanner_v5";
const loadS = () => {
  try { const r = localStorage.getItem(SKEY); if(r) return JSON.parse(r); } catch(e) {}
  return {
    types: TYPES0, tasks: INIT_TASKS,
    eduItems: [{id:gid(),target:"worker",type:"center",nth:1,startDate:"2026-03-25",endDate:"2026-03-26",startTime:"09:00",endTime:"17:00",region:"",place:"",lectures:[{id:gid(),subject:"식품위생 기초",instructor:"김강사"}],note:""}]
  };
};
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button onClick={onClose} style={{border:"none",background:"none",fontSize:20,cursor:"pointer",lineHeight:1}}>×</button>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <div style={{flex:1}}>
            ...
            <select value={f.priority} onChange={e=>upd("priority",parseInt(e.target.value))} style={{width:"100%",padding:"6px 8px",border:"1px solid #ccc",borderRadius:8}}>
              {PL.map((l,i)=><option key={i} value={i}>{l}</option>)}
            </select>
          </div>
...
function EduForm({initial,eduItems,onSave,onClose,onDelete}) {
  const isEdit = !!onDelete;
  const all = eduItems||[];
  const aNth = (tgt,cid,sd) => {
    const same = all.filter(e=>e.target===tgt&&e.id!==cid).sort((a,b)=>a.startDate>b.startDate?1:-1);
    const idx = same.findIndex(e=>e.startDate>(sd||"9999"));
  ...
  const upd = (k,v) => setF(p => {
    if(k==="target"&&!isEdit) return {...p,[k]:v,nth:aNth(v,null,p.startDate)};
    if(k==="startDate"&&!isEdit) return {...p,[k]:v,nth:aNth(p.target,null,v)};
    return {...p,[k]:v};
  });
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        {f.type==="visit"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:10,borderRadius:8,background:rgba(ti.color,0.07),border:`1px dashed ${rgba(ti.color,0.4)}`}}>
            <div><div style={{fontSize:11,color:"#666",marginBottom:3}}>지역</div><input value={f.region} onChange={e=>upd("region",e.target.value)} placeholder="예) 충북" style={{width:"100%",padding:"6px 8px",border:"1px solid #ccc",borderRadius:8,fontSize:13,boxSizing:"border-box"}}/></div>
            <div><div style={{fontSize:11,color:"#666",marginBottom:3}}>장소</div><input value={f.place} onChange={e=>upd("place",e.target.value)} placeholder="예) 청주교육원" style={{width:"100%",padding:"6px 8px",border:"1px solid #ccc",borderRadius:8,fontSize:13,boxSizing:"border-box"}}/></div>
          </div>
        )}
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{fontSize:11,color:"#666",whiteSpace:"nowrap"}}>차수</div>
          <input type="number" min={1} value={f.nth} onChange={e=>upd("nth",parseInt(e.target.value)||1)} style={{width:70,padding:"6px 8px",border:"1px solid #ccc",borderRadius:8}}/>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
            <div style={{fontSize:11,color:"#666"}}>강의 내용 및 강사</div>
            <button onClick={()=>upd("lectures",[...f.lectures,{id:gid(),subject:"",instructor:""}])} style={{fontSize:11,padding:"2px 8px",background:rgba(ti.color,0.1),border:`1px solid ${ti.color}`,color:ti.color,borderRadius:10,cursor:"pointer"}}>+ 추가</button>
          </div>
          {f.lectures.map((l,i)=>(
            <div key={l.id} style={{display:"flex",gap:5,alignItems:"center",marginBottom:5,padding:"6px 8px",background:"#f9f9f9",borderRadius:8}}>
              <span style={{fontSize:11,color:"#999",minWidth:18}}>{i+1}.</span>
              <input placeholder="강의 과목" value={l.subject} onChange={e=>upd("lectures",f.lectures.map(x=>x.id===l.id?{...x,subject:e.target.value}:x))} style={{flex:2,padding:"5px 8px",border:"1px solid #ccc",borderRadius:6,fontSize:12}}/>
              <input placeholder="강사명" value={l.instructor} onChange={e=>upd("lectures",f.lectures.map(x=>x.id===l.id?{...x,instructor:e.target.value}:x))} style={{flex:1,padding:"5px 8px",border:"1px solid #ccc",borderRadius:6,fontSize:12}}/>
              {f.lectures.length>1&&<button onClick={()=>upd("lectures",f.lectures.filter(x=>x.id!==l.id))} style={{border:"none",background:"none",color:"#999",fontSize:14,cursor:"pointer"}}>×</button>}
            </div>
          ))}
        </div>
...
function TaskList({types,tasks,onTask,onAdd,onBulk}) {
  ...
  const list=tasks
    .filter(t=>filter==="all"||(filter==="active"?t.status==="before"||t.status==="doing":filter===t.status))
  ...
  const apply = () => {
    const ch={};
    if(bType) ch.type=bType;
    if(bStat) ch.status=bStat;
    if(bPri!=="") ch.priority=parseInt(bPri);
...
function App() {
  const [state,setState] = useState(loadS);
  const [hist,setHist] = useState([]);
  const {types,tasks,eduItems} = state;
  ...
  useEffect(()=>{saveS(state);},[state]);

  const showT = (msg) => {setToast(msg);clearTimeout(tRef.current);tRef.current=setTimeout(()=>setToast(null),2200);};
  const saveTask = (form) => {if(modal?.type==="edit")push({...state,tasks:tasks.map(t=>t.id===modal.task.id?{...t,...form}:t)});else push({...state,tasks:[...tasks,{id:gid(),...form}]});showT("저장됨");setModal(null);};
  const delTask = (id) => {push({...state,tasks:tasks.filter(t=>t.id!==id)});showT("삭제됨");setModal(null);};
  const saveTypes = (t) => {push({...state,types:t});showT("유형 저장됨");setModal(null);};
  const bulkUpd = (ids,ch) => {push({...state,tasks:tasks.map(t=>ids.includes(t.id)?{...t,...ch}:t)});showT(`${ids.length}개 수정됨`);};
    ...
    <div style={{padding:mobile?"0.75rem":"1.5rem",minHeight:"100vh",background:"#f0f0f0",boxSizing:"border-box"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={()=>{if(hist.length){undo();showT("되돌렸어요");}}} style={{fontSize:12,padding:"5px 10px",borderRadius:20,border:"1px solid #ddd",background:hist.length?"#fff":"#f0f0f0",color:hist.length?"#1a1a1a":"#bbb",cursor:hist.length?"pointer":"default"}}>↩{hist.length>0?` (${hist.length})`:""}</button>
        </div>
